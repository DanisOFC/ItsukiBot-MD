       ...
       if (opts['queque'] && m.text && !(isMods || isPrems)) {
		...
		//m.reply('*ERROR DE COMANDO*')}
            ...
            if (!['unbanuser.js', 'inv.js', 'link.js', 'creator.js', 'profile.js'].includes(name) && user && user.banned && !isROwner) {
                    if (!opts['msgifbanned']) m.reply(`*Estas baneado* ${user.bannedReason ? `\n*Motivo:* *${user.bannedReason}*` : ''}

*Puedes contactar al dueño de la Bot si crees que se trata de un error (TENER PRUEBAS) para tratar el motivo de tú desbaneo*
`.trim())
                        return
                }
                }
